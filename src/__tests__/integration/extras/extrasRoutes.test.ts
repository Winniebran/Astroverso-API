...
describe("/extras", () => {
  ...
  test("POST /extras - Should not be able to create an extra that already exists", async () => {
    const admLoginResponse = await request(app)
      .post("/login")
      .send(mockAdmLogin);
    const extrasId = await request(app).get("/types").send(mockTypes);
    const response = await request(app)
      .post("/extras")
      .set("Authorization", `Bearer ${admLoginResponse.body.token}`)
      .send({ ...mockExtras, typesId: extrasId.body[0].id });
  ...
  test("PATCH /extras/:id -  should be able to update extras", async () => {
    const extrasId = await request(app).get("/types").send(mockTypes);
    const updatedExtra = {
      image: "https://teste5.com",
      author: "Nome do novo autor",
      title: "Nome do novo título",
      description: "Nova descrição",
      link: "https://teste6.com",
      types: { id: extrasId.body[0].id },
    };

    const admLogin = await request(app).post("/login").send(mockAdmLogin);
    const extrasToBeUpdated = await request(app)
      .get("/extras")
      .set("Authorization", `Bearer ${admLogin.body.token}`);

    const response = await request(app)
      .patch(`/extras/${extrasToBeUpdated.body[0].id}`)
      .set("Authorization", `Bearer ${admLogin.body.token}`)
      .send(updatedExtra);

    const findExtra = await request(app)
      .get("/extras")
      .set("Authorization", `Bearer ${admLogin.body.token}`);
      expect(response.status).toBe(200);
      expect(findExtra.body[0].image).toEqual("https://teste5.com"),
      expect(findExtra.body[0].author).toEqual("Nome do novo autor"),
      expect(findExtra.body[0].title).toEqual("Nome do novo título"),
      expect(findExtra.body[0].description).toEqual("Nova descrição"),
      expect(findExtra.body[0].link).toEqual("https://teste6.com"),
      expect(findExtra.body[0].types.id).toEqual(extrasId.body[0].id);
  });
  ...
  test("PATCH /extras/:id -  should not be able to update extras not being admin", async () => {
    ...
    const response = await request(app)
      .patch(`/extras/${userTobeUpdate.body[0].id}`)
      .set("Authorization", `Bearer ${userLoginResponse.body.token}`);
  ...
  test("DELETE /extras/:id -  should not be able to delete extras not being admin", async () => {
    ...
    const response = await request(app)
      .delete(`/extras/${UserTobeDeleted.body[0].id}`)
      .set("Authorization", `Bearer ${userLoginResponse.body.token}`);
  ...
  test("DELETE /extras/:id -  should not be able to delete extras with invalid id", async () => {
    const adminLoginResponse = await request(app).post("/login").send(mockAdm);
    const response = await request(app)
      .delete(`/extras/13970660-5dbe-423a-9a9d-5c23b37943cf`)
      .set("Authorization", `Bearer ${adminLoginResponse.body.token}`);
