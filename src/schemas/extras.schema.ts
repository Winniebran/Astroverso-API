...
export const extrasResponseSchema: SchemaOf<IExtrasRequest> = yup
  .object()
  .shape({
    image: yup.string().url().required("Image is required"),
    author: yup.string().required("Author is required"),
...
export const extrasUpdateSchema: SchemaOf<IExtrasUpdate> = yup.object().shape({
  ...
  typesId: yup.string().notRequired(),
});

export const listExtrasSchema = yup.array(extrasResponseSchema);
